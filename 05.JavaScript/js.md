...
基本数据类型： Undefined、Null、Boolean、Number、String，Symbol, BigInt, 还有在 ES6 中新增的 Symbol 类型，代表创建后独一无二且不可变的数据类型，它的出现我认为主要是为了解决可能出现的全局变量冲突的问题。
引用数据类型: Object,Array,Function, Date, Reg
...
> 返回一个数组,包括**对象自身**的所有可枚举属性(不含继承的, 不含 Symbol 属性)
...
> 循环遍历**对象自身的和原型**上继承的可枚举属性(不含 Symbol 属性).
...
1. 普通对象都包含`__proto__`属性,指向对象的原型对象
2. 函数对象都包含`__proto__`和 `prototype`属性，指向对象的原型对象
3. 原型对象包含 constructor 指向对象的构造函数

**原型链**

当获取某个对象的属性时，首先会从对象自身的属性中查找是否存在，如果不存在就从对象的原型对象上查找，如果还是没找到继续想原型对象的原型对象上查找，直至 Object 原型对象的原型对象 null 才结束。
