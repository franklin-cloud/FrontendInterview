...
## 142. [环形链表 Ⅱ](https://leetcode-cn.com/problems/linked-list-cycle-ii)
...
var maxSlidingWindow = function (nums, k) {
  class slideWindow {
    ...
    push(val) {
      // 队列从尾部开始，把小于val的数都去掉，然后将val加入队列末尾
...
var calcEquation = function (equations, values, queries) {
  ...
  keys.forEach((x) => {
    keys.forEach((y) => {
      map[x][y] &&
        keys.forEach((z) => {
          // 如果 x/y 有值 y/z 也有值，则 x/z 的值也可以求出
          if (map[y][z]) {
...
var longestMountain = function (A) {
  ...
  for (let i = 1; i < len - 1; i++) {
    ...
    if (A[i] > A[i - 1] && A[i] > A[i + 1]) {
      ...
      for (let j = i + 1; j < len && A[j] < A[j - 1]; j++) {
        count++;
      }
      count++;
    }
...
var middleNode = function (head) {
  ...
  while (len > 1) {
    // 这里注意是大于1，比如len=3，head只需要往后移两次，1->2,2->3
    head = head.next;
    len--;
  }
  return head;
